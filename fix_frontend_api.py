...
export default function App() {
  ...
  const [logs, setLogs] = useState([
    ...
    "[STATUS] Autonomous agent loop ready for live code modification."
  ]);
  ...
  const handleSendCommand = async (e) => {
    ...
    try {
      ...
      const reply = data.response || "[SYSTEM] Task processed.";
      setLogs((prev) => [...prev, `[JARVIS] ${reply}`]);

      // If backend updated the file, Vite HMR will instantly auto-reload the layout
    } catch (err) {
      setLogs((prev) => [...prev, "[JARVIS ERROR] Backend API unreachable. Check terminal server."]);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div style={{
      ...
      <header style={{
        ...
        <div style={{ textAlign: 'right', fontSize: '0.85rem' }}>
          ...
          <div style={{ color: '#10b981', marginTop: '0.2rem' }}>STATUS: ● ACTIVE</div>
        </div>
      </header>
        ...
        <section style={{
          ...
          <div style={{ overflowY: 'auto', maxHeight: '400px', fontSize: '0.85rem' }}>
            ...
            {loading && (
              <div style={{ color: '#f59e0b', marginTop: '0.5rem' }}>
                &gt; [JARVIS AGENT] Processing autonomous execution loop...
              </div>
            )}
          </div>
          ...
          <form onSubmit={handleSendCommand} style={{ display: 'flex', gap: '0.5rem', marginTop: '1rem' }}>
            ...
            <button type="submit" disabled={loading} style={{
              backgroundColor: loading ? '#64748b' : '#00f3ff',
              color: '#030712',
              fontWeight: 'bold',
              border: 'none',
              borderRadius: '4px',
...
print("[SUCCESS] Connected Frontend Form to Real-time Backend API Engine!")